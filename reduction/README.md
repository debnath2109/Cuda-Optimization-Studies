...
| 5 | `cub::DeviceReduce::Sum` | 0.692 ms | 97% | 17% | 2% | (library) |
| 6 | `thrust::reduce` | 0.583 ms | 93% | 39% | 15% | (library) |
...
## Validation against production libraries

To establish what "good" means in absolute terms, the hand-tuned kernel was benchmarked
against NVIDIA's own `cub::DeviceReduce::Sum` and `thrust::reduce` on the same 32M input.

All three converge at **93–97% DRAM throughput**, confirming the central finding: this
problem is **bandwidth-bound**, and ~97% is the true hardware ceiling — not an artifact of
any one implementation. When the specialist kernel, CUB, and Thrust all hit the same wall,
that wall is the memory bus.

The hand-tuned kernel matches CUB's bandwidth utilization (both 97% DRAM). CUB's slightly
higher absolute time reflects the cost of a fully general implementation — arbitrary sizes,
types, and operators, with robust parameter selection across all GPUs — versus a specialist
kernel tuned for exactly this case. The profiler also reveals *how* the libraries work:
Thrust dispatched through a general `for_each`-style path (93% DRAM), while CUB used a
dedicated `DeviceReduceKernel` with explicit even work-sharing across the grid (97% DRAM).

The takeaway is not "faster than the library" — it is that the kernel **reaches the same
hardware ceiling the production libraries do**, verified independently.
