...
| 2 | Shared-memory tree | 56.8 µs | 28% | 88% | 36× |
| 3 | Grid-stride + warp shuffle | 36.8 µs | 57% | 51% | 56× |
...
atomic per block. Nsight revealed the new bottleneck: **L1 at 88%, DRAM only 28%** — the
kernel was limited by shared-memory traffic and `__syncthreads` overhead, not bandwidth.
...
eliminating shared memory and barriers for the last warp. DRAM rose 28% → 57% as the
bottleneck shifted toward memory.

**4. Vectorized float4 loads (24.3 µs, 84×).** Each thread loads 128 bits (four floats)
per instruction. **DRAM throughput reached 86%** — the practical ceiling for a memory-bound
...
\`\`\`bash
nvcc -arch=sm_89 float4Optimized.cu -o float4Optimized
ncu --set full -o report ./float4Optimized   # profile with Nsight Compute
\`\`\`
